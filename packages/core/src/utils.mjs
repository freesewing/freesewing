 ...
 * @param {Point} b1 - Point 1 of line B
 * @param {Point} b2 - Point 2 of line B
 * @return {{p:Point, t: number, u:number}|false} intersections - The intersection result or `false` if the lines are parallel. p is the point of the intersection, t and u determine where the intersection lies relative to the points of line A and line B respectively
 */
function beamIntersection(a1, a2, b1, b2) {
  ...
  if (Math.abs(rCrossS) < EPSILON) {
    return false // The lines are parallel (or almost parallel)
  }
  ...
  // Compute the intersection point using a1 + t * r
  return {
 ...
 * @param {Point} b1 - Point 1 of line B
 * @param {Point} b2 - Point 2 of line B
 * @return {{p:Point, t: number, u:number}|false} intersections - The intersection result or `false` if the lines are parallel
 */
export function beamsIntersectAdvanced(a1, a2, b1, b2) {
  // Function to compute the cross product of two vectors
  function crossProduct(v1, v2) {
    return v1.x * v2.y - v1.y * v2.x
  }
...
export function linesIntersect(a1, a2, b1, b2) {
  const intersection = beamIntersection(a1, a2, b1, b2)
  if (!intersection) return false
  if (intersection.t < 0 || intersection.t > 1) return false // outside of line segment A
  if (intersection.u < 0 || intersection.u > 1) return false // outside of line segment B
  return intersection.p
}
...
export function beamIntersectsLine(a1, a2, b1, b2) {
  const intersection = beamIntersection(a1, a2, b1, b2)
  if (!intersection) return false
  if (intersection.u < 0 || intersection.u > 1) return false // outside of line segment
  return intersection.p
}
