export default part => {
  ...
  const box = (name, origin, width, height, tolerance = 10) => {
    ...
    points[name + "TopLeft"] = new Point(
      about(origin.x, t),
      about(origin.y, t)
    );
    points[name + "BottomLeft"] = new Point(
      about(origin.x, t),
    ...
    points[name + "TopRight"] = new Point(
      about(origin.x + width, t),
      about(origin.y, t)
    );
    ...
    return new Path()
      ...
      .curve(
        points[name + "TopRightCp2"],
        points[name + "TopLeftCp2"],
        points[name + "TopLeft"]
      )
      .close()
  ...
  const arrow = (name, text = "", tolerance = 10) => {
    ...
    let f = 0.3;
    let r = tolerance / 2;
    points[name + "FromCp"] = from
      .shiftFractionTowards(to, about(f))
      .rotate(nabout(r), from);
    points[name + "ToCp"] = to
      .shiftFractionTowards(from, about(f))
      .rotate(nabout(r), to);
    points[name + "Tip1"] = to
      .shiftTowards(from, about(3.8))
      .rotate(about(15), to);
    points[name + "Tip2"] = to
      .shiftTowards(from, about(3.5))
      .rotate(about(-15), to);
    let path = new Path()
      .move(from)
  ...
  points.arrow1To = points.PatternTopLeft.shiftFractionTowards(
    points.PatternBottomLeft,
    0.5
  );
  paths.arrow1 = arrow("arrow1");
  points.arrow2From = points.PatternTopRight.shiftFractionTowards(
    points.PatternBottomRight,
    0.5
  );
  points.arrow2To = points.draftTopLeft.shiftFractionTowards(
    points.draftBottomLeft,
    0.5
  );
  ...
  points.arrow3From = points.draftTopLeft.shiftFractionTowards(
    points.draftTopRight,
    0.5
  );
  points.arrow3To = points.svgText.clone();
  paths.arrow3 = arrow("arrow3", "render()");
  points.arrow4From = points.draftBottomLeft.shiftFractionTowards(
    points.draftBottomRight,
    0.5
  );
