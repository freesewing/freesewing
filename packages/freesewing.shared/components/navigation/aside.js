...
const plainClasses = "px-1 py-1 leading-6"
    ...
    <div className='flex flex-row'>
      ...
      >
        {(props.level < 5 && hasChildren(props.branch)) ? (
          <button className="pt-1" onClick={() => toggleActive(props)}>
            <Icon icon='down' size={(24 - props.level*2)} className={iconClasses} />
          </button>
        ) : (
          <button className="mt-2">
            <Icon icon='middot' size={(24 - props.level*2)} className={iconClasses} />
          </button>
          )}
      </div>
