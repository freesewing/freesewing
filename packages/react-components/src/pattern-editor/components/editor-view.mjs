...
export const EditorView = (props) => {
  // Swizzled defaults
  const { ui: _ui } = useDefaults(props.defaults)
  ...
  const [ui, setUi] = useState(props.preloadUi || _ui)
...
const viewfinder = ({
   ...
   * Do we have a design?
   */
  if (!designs[design] && !designs[state.design])
    return [views.designs, { designs, locale, t, setDesign: state.setDesign }]
