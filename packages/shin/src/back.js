export default function(part) {
  ...
  store.set(
    "hips",
    (measurements.hipsCircumference / 2) * utils.stretchToScale(options.stretch)
  );
  store.set("hipFront", store.get("hips") * options.frontFactor);
  store.set("hipBack", store.get("hips") * (1 - options.frontFactor));
  store.set(
    "legs",
    measurements.upperLegCircumference * utils.stretchToScale(options.stretch)
  );
  ...
  points.legSideCp = points.legSide.shift(
    options.angle * -1 + 7,
    store.get("legBack")
  );
  points.legInner = points.legSideCp.shift(
    options.angle * -1 - 90,
    store.get("gusset") / 2
  );
  ...
  points.crossSeam = points.legInner.shift(
    gussetAngle - 90,
    store.get("gusset")
  );
