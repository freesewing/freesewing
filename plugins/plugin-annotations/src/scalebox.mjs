...
export const scaleboxMacros = {
  scalebox: function (so, { store, points, paths, scale, Point, Path, complete }) {
    let prefix
    if (so.id) {
      prefix = '_' + so.id
    } else {
    ...
    if ((complete && so.detail) || !so.detail) {
      ...
      // Box points
      points[id + 'MetricTopLeft'] = new Point(
        so.at.x - metricWidth / 2,
        so.at.y - metricHeight / 2
      )
      points[id + 'MetricTopRight'] = new Point(
        so.at.x + metricWidth / 2,
        so.at.y - metricHeight / 2
      )
  ...
  miniscale(so, { points, paths, scale, Point, Path, complete }) {
    let prefix
    if (so.id) {
      prefix = '_' + so.id
    } else {
