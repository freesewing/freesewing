...
You can disable the front and back pockets, for example if you want to create a simple mockup for
...
The pocket entry facing is a thin strip of fabric on the top edge, inside the pockets.

Disabling them can be useful if you're making the pockets out of the same material as the outer layer of your pants
(so the pocket facing would be unnecessary).
Or if you're going for a specific look where the pocket bag material should be visible from the outside for added contrast.
...
The last main option determines if you want a crotch gusset.
...
so you'll need to finish (serge or zigzag) them, even if you're using a serger for the seams later.
...
:::note
If you use fabric that frays and don't want to serge the other seams,
you may also want to finish all the remaining raw edges now, so you don't have to do that later.
:::
...
Edge stitch (understitch) a millimeter away from the seam to secure the seam allowance.
...
Add some bartacks to secure the fly bottom.
...
where the yokes and the waistband meet.
...
Press the seam allowance towards the back or towards each side if you've kept them separate.

With the good side on top, reinforce the outseam on the back part by stitching on the back part close
to the side seam from the waist down until the pocket bag ends or about 15 cm (6 inches).

Make sure to catch the pocket bag and the seam allowance in the seam.
...
Do this on both sides, making sure both sides have the same length.
