...
const getColor = (a, b, color, tint = false) => colors[color][tint ? tint : 500]
const colorDot = (dot) => dot.replace(regexA, getColorTint).replace(regexB, getColor)
const fixSvg = (svg) => svg.replace(/#000000/g, () => 'currentColor')
const wrapDot = (dot, plain = false) => {
...
const Dot = ({ children, caption, eng = 'dot' }) => {
  ...
  const dot = wrapDot(asText(children), plain)
        ...
        <figure className="py-2">
          <div className="flex flex-row justify-center gap-8 mb-4">
            <div className="flex items-center gap-1">
              ...
              />
              <span style={{ fontFamily: 'Indie Flower' }} className="font-bold text-sm">
                sketch
              </span>
            </div>
            <div className={`dropdown dropdown-end`}>
              ...
              <ul
                tabIndex="0"
                className="shadow menu dropdown-content bg-base-100 rounded-lg border p-0 py-2 pl-0"
              >
                {engines.map((eng) => (
                  <li key={eng}>
                    <button
