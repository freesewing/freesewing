...
export const PatronsCta = ({ color = 'secondary' }) => {
  ...
  const { amounts, periods, currencies } = freeSewingConfig.patrons.presets
  ...
  return (
    <div className="w-full">
      {submit ? (
        <>
          <p className="text-inherit">
            Oh wow, I appreciate the enthusiasm, but for now this is merely a mockup as we are
            integrating with Stripe as our new payment processor
          </p>
          <p className="text-inherit">
            If you like, you can{' '}
            <a
            ...
            >
              sign-up as a patron on the current production website
            </a>{' '}
            or wait until we release version 3 and this website in production.
          </p>
        </>
      ) : (
        <>
      ...
      <button className={`btn btn-${color} w-full mt-4`} onClick={() => setSubmit(!submit)}>
        {submit ? 'Back to form' : 'To checkout'}
      </button>
    </div>
  )
}
