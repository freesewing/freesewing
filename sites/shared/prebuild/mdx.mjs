...
// Header for auto-generate pages
const header = `/*
...
/*
 * Set up the available locales for various sites
 */
const locales = {
  dev: ['en'],
  lab: ['en', 'fr', 'es', 'nl', 'de'],
...
/**
 * Reads and returns a page template file as a string
 */
function readPageTemplate(site, template) {
  return fs.readFileSync(
    path.resolve('..', '..', 'sites', site, 'page-templates', template)
    , 'utf-8'
  )
}

/**
 * Creates a NextJS page from a template
 */
function createPage(site, slug, template, replace) {
  const dir = path.resolve('..', '..', 'sites', site, 'pages', slug)
  return fs.mkdir(dir, { recursive: true }, (err) => {
    if (err) console.log(err)
    return fs.writeFileSync(
      path.resolve(dir, 'index.js'),
      header + mustache.render(template, replace)
    )
  })
}
...
export const prebuildMdx = async(site) => {
  ...
  // Load page template
  const template = readPageTemplate(site, 'mdx.js')

  // Loop over locales
  const pages = {}
  for (const lang of locales[site]) {
    ...
    for (const file of list) {
      ...
      if (lang === 'en') {
        // Write out nextjs page from template
        createPage(site, slug, template, { slug })
      }
    }
