...
export const prebuildStrapi = async (site) => {
  ...
  // Say hi
  console.log()
  console.log(`Prebuilding Strapi content for ${site}`)
  ...
  for (const type of types) {
    ...
    for (const lang of (site === 'dev' ? ['en'] : ['en', 'es', 'de', 'fr', 'nl'])) {
      ...
      fs.writeFileSync(
        path.resolve('..', site, 'prebuild', `strapi.${type}.${lang}.js`),
      ...
      for (const [slug, post] of Object.entries(posts[type][lang])) {
        fs.writeFileSync(
          path.resolve('..', site, 'prebuild', `strapi.${type}.${lang}.${slug}.json`),
      ...
      for (const [slug, author] of Object.entries(authors[type][lang])) {
        fs.writeFileSync(
          path.resolve('..', site, 'prebuild', `strapi.authors.${type}.${lang}.${slug}.json`),
          JSON.stringify({
            slug: author.slug,
            name: author.name,
