...
export const valToImperialFraction = (fraction, format='html') => {
  let negative = ''
  let inches = ''
  let rest = ''
  if (fraction < 0) {
  ...
  return negative + Math.round(fraction * 100) / 100 + (['notags', 'html'].indexOf(format) > -1 ? '"' : '')
}
...
export const mmToInches = (value) => value * 25.4
export const mmToCm = (value) => value * 10;
